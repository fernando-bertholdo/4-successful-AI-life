...
echo "-- CC-generated auto-title (e.g. <local-command-caveat>... (Branch 2)) is NOT promoted --"
# Reproduces the bug seen in session 93e377b3-…/projeto-gama: CC auto-named the session
# from a synthetic slash-command pseudo-user-message and the plugin froze that as override.
tspu=$(mktemp).jsonl
cp "$SCRIPT_DIR/../fixtures/transcript-v15-feature.jsonl" "$tspu"
spurious_title='<local-command-caveat>Caveat: The messages below were generated by the user while running local comm (Branch 2)'
jq -nc --arg t "$spurious_title" '{type:"custom-title", customTitle:$t}' >> "$tspu"
run_hook "$tspu" "sess-spurious"
s=$(cat "$CLAUDE_PLUGIN_DATA/state/sess-spurious.json")
assert_eq "no override set" "" "$(echo "$s" | jq -r '.manual_title_override // ""')"
assert_eq "rendered_title not poisoned" "" "$(echo "$s" | jq -r '.rendered_title // ""' | head -c 22)"
assert_eq "tombstoned in last_plugin_written_title" "$spurious_title" "$(echo "$s" | jq -r '.last_plugin_written_title // ""')"

echo "-- Tombstone suppresses repeat on next Stop hook (no new spurious_custom_title_ignored event) --"
log_file="$CLAUDE_PLUGIN_DATA/logs/sess-spurious.jsonl"
events_before=$(grep -c spurious_custom_title_ignored "$log_file" 2>/dev/null || echo 0)
run_hook "$tspu" "sess-spurious"
events_after=$(grep -c spurious_custom_title_ignored "$log_file" 2>/dev/null || echo 0)
assert_eq "no repeat warn" "$events_before" "$events_after"
