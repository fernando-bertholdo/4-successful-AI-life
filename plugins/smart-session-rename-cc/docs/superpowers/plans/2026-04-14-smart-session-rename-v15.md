...
`-Users-dev-Documents-tech-projects-claude-code-smart-session-rename`.
Pick the most recently modified JSONL in that directory (= the current session):

```bash
PROJECT_ENCODED="-Users-dev-Documents-tech-projects-claude-code-smart-session-rename"
...
The raw transcript may contain absolute paths (`/Users/dev/…`),
...
sed -i.bak 's#/Users/dev#/Users/<user>#g' tests/fixtures/transcript-real-capture.jsonl
...
- Dev-install this repo as a plugin. Two common ways:
  ...
  - Export `CLAUDE_PLUGIN_ROOT` env to this repo before launching `claude`:
    ```bash
    export CLAUDE_PLUGIN_ROOT=/Users/dev/Documents/tech_projects/claude-code-smart-session-rename
    claude
    ```
