...
Version `0.2.1` · 1 skill · native plugin
...
- **What wakes an agent when you comment.** The implicit routing the reference omits: `/note` as the first token triggers nothing, `@all` cancels the assignee fallback, a top-level comment with no mention wakes the issue's agent assignee **in any status including a closed one**, and `issue comment add` has no `--no-start`.
