...
### Changed
- `multica-ops` `0.2.8` → `0.2.9` (marketplace `0.8.10` → `0.8.11`): `scripts/patch-description.py` decides the window by counting `description_updated` before the read and after the write — not by `revision` +1, which comments and status changes also move, nor by `--since <updated_at>`, which drops the very second that matters; exit 3 says the erased text cannot be recovered through the CLI and prints each event in the window with its time and author, so that author can be asked to re-apply the change; inside an agent task it takes `MULTICA_AGENT_ID` as its own id; `--edits` takes only pairs of two strings; and a test suite against a fake CLI — see the [plugin changelog](./plugins/multica-ops/CHANGELOG.md).
- `prompt-master` `1.0.0+upstream-1.7.0` → `1.0.0+upstream-1.8.0` (marketplace `0.8.9` → `0.8.10`): weekly upstream sync, `nidhinjs/prompt-master` `v1.7.0` → `v1.8.0` (HEAD `2bd9251`), a current-model routing refresh. No local patches to re-apply. See [`plugins/prompt-master/CHANGELOG.md`](./plugins/prompt-master/CHANGELOG.md).
- `multica-ops` `0.2.7` → `0.2.8` (marketplace `0.8.8` → `0.8.9`): a description write replaces the whole field and the CLI has no compare-and-set; three habits against the silent loss, and `scripts/patch-description.py`, which checks `revision` for the write that lands before yours — see the [plugin changelog](./plugins/multica-ops/CHANGELOG.md).
...
- `multica-ops` `0.2.2` → `0.2.3` (marketplace `0.8.3` → `0.8.4`): the "53 s" in the pre-merge note was a hand-off figure; primary sources give 2 s to the conductor's next run and 150 s to the first follow-up issue.
- `multica-ops` `0.2.1` → `0.2.2` (marketplace `0.8.2` → `0.8.3`): the recipes two operating sessions had to discover by failing — lenient JSON parsing by default (typed string fields carry raw control characters intermittently, measured on CLI v0.4.44), manifest ids read by the key the manifest uses (`title:` for projects), board writes never chained to a fallible step, `issue runs` + `issue timeline` before merging a human-conducted PR (on 18/09/2026 a human merged while the conductor agent was still running rounds on the same PR; the stage barrier woke the conductor 2 s after the merge, the verdict 52 s after, and it opened two follow-up issues at 150 s and 158 s — measured at the source in 0.2.4, after 0.2.3 had swapped a correct "53 s" for the wrong event), and `/note` addresses nothing to anyone. See [`plugins/multica-ops/CHANGELOG.md`](./plugins/multica-ops/CHANGELOG.md).
- `multica-ops` `0.2.0` → `0.2.1` (marketplace `0.8.1` → `0.8.2`): a section on what wakes an agent when you comment. Verified against Multica's product docs and the server's comment-routing code — `/note` as the first token suppresses every trigger, `@all` cancels the assignee fallback, a top-level comment with no mention wakes the issue's agent assignee in **any** status including a closed one, a plain reply to a member's comment does not, and `issue comment add` has no `--no-start` despite a widely-repeated answer to the contrary.
- `multica-ops` `0.1.0` → `0.2.0`, re-cut as a companion to Multica's own CLI skill after a measured comparison (marketplace `0.8.0` → `0.8.1`). [`multica-ai/multica-cli`](https://github.com/multica-ai/multica-cli) is linted daily against the real binary; running that linter against CLI v0.4.42, four weeks and sixteen patch versions after its last commit, surfaced only two divergences — so it is the command reference, and everything it covers (`issue search`, comment read modes, the working-directory restriction on file arguments, mandatory `--parent`, `mention://` side effects, metadata vs. custom properties, subscribers, run inspection, stages, PR linking) was removed from ours rather than restated. What remains is what it does not cover: desktop-app binary discovery (where `command -v multica` finds nothing and the reference's first step fails), the bridged Cowork/claude.ai route, the zsh word-splitting bugs, the JSON shapes and cost flags the reference omits (`--limit` caps at 100, `has_more` paging, `--fields`, `issue timeline`, the `*-stdin` family), the platform limits (workspace isolation, artifacts disabled inside agent tasks, `agent update --mcp-config`, cron autopilots), and a new governance section for boards governed by a repository that versions their Multica configuration.

### Added
- Sixth plugin: `multica-ops` at `v0.1.0` (see [`plugins/multica-ops/CHANGELOG.md`](./plugins/multica-ops/CHANGELOG.md)). Native, one skill (`board`): the operating manual for a Multica board from Claude Code, or from a Cowork/claude.ai session bridged to the machine running the desktop app. Discovery-first by design — the profile comes from `~/.multica/profiles`, workspaces from `workspace list`, every other id from the matching `list` command — so **no workspace id, project name or machine path ships in the public repo**; a private configuration repo can be pointed at via `MULTICA_PLAYBOOK` and the skill defers to its manifests and deploy tooling. Documents safe defaults (`--no-start` on every `issue status`, silence on unscoped issues, promotion and deploy reserved for a human, no casual `create`, a dated trace when a session acts under a human's profile), the CLI output shapes that are not arrays, the positional label argument, the 100-item page cap, the zsh word-splitting bugs behind the `mt()` wrapper, and the platform limits (per-workspace isolation, artifacts disabled inside agent tasks, no MCP surface). Measured against CLI v0.4.42. Marketplace version bumped `0.7.0` → `0.8.0`.
